...
import { detectServiceType } from '@/lib/x402'
import { TryAgentModal } from './try-agent-modal'
...
export function AgentCard({ agent }: AgentCardProps) {
  ...
  const hasService = !!agent.endpoint && !!detectServiceType(agent.endpoint)
  ...
  return (
    ...
    >
      ...
      <div className="px-3 pt-3 pb-3 flex flex-col gap-2.5 flex-1 min-h-0 min-w-0">
        ...
        {/* Bonding progress */}
        <ProgressBar percent={0} graduated={agent.tokenGraduated} />
      </div>
      ...
      {/* Try Agent Modal — rendered outside the link via portal-like pattern */}
      {hasService && (
        <TryAgentModal
          open={tryModalOpen}
